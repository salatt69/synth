...
namespace ProjectSynth.Modules.Characters
{
    public abstract class SurvivorBase<T> : CharacterBase<T> where T : SurvivorBase<T>, new()
    {
        ...
        protected virtual void InitializeSurvivor()
        {
            ...
            survivorDef.primaryColor = bodyInfo.bodyColor;

            survivorDef.cachedName = bodyPrefab.name.Replace("Body", "");
            survivorDef.displayNameToken = survivorTokenPrefix + "NAME";
            survivorDef.descriptionToken = survivorTokenPrefix + "DESCRIPTION";
            survivorDef.outroFlavorToken = survivorTokenPrefix + "OUTRO_FLAVOR";
            survivorDef.mainEndingEscapeFailureFlavorToken = survivorTokenPrefix + "OUTRO_FAILURE";

            survivorDef.desiredSortPosition = bodyInfo.sortPosition;
            survivorDef.unlockableDef = characterUnlockableDef;
